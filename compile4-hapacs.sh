...
#------------------
INCLUDE=/opt/MPI/MVAPICH2/2.2/PGI_medium/16.10/CUDA-8.0.44/include/
...
xmpf90  -DARTED_CURRENT_OPTIMIZED -DARTED_LBLK -DARTED_SC -DARTED_STENCIL_OPTIMIZED -DARTED_STENCIL_PADDING -omp -cpp -Minfo=acc -xacc --Wn-acc --Wl-acc -I${INCLUDE} -D_OPENMP -ta=tesla,cc35,ptxinfo,maxregcount:128 -Mpreprocess -Kieee -c xacc.f90 -v
if [ $? -ne 0 ]; then
    exit 1
fi
XMP_ONLYCAF=1 xmpf90  -DARTED_CURRENT_OPTIMIZED -DARTED_LBLK -DARTED_SC -DARTED_STENCIL_OPTIMIZED -DARTED_STENCIL_PADDING -cpp -Minfo=acc -xacc --Wn-acc --Wl-acc -I${INCLUDE} -D_OPENMP -ta=tesla,cc35,ptxinfo,maxregcount:128 -Mpreprocess -Kieee -c xacc-noomp.f90 -omp
if [ $? -ne 0 ]; then
    exit 1
fi
...
xmpf90 -DARTED_CURRENT_OPTIMIZED -DARTED_LBLK -DARTED_SC -DARTED_STENCIL_OPTIMIZED -DARTED_STENCIL_PADDING -omp -cpp -Minfo=acc -xacc --Wn-acc --Wl-acc -I${INCLUDE} -D_OPENMP -ta=tesla,cc35,ptxinfo,maxregcount:128 -Mpreprocess -Kieee xacc.o xacc-noomp.o env_variables_internal.o papi_wrap.o -llapack -lblas ./main/sc.f90
if [ $? -ne 0 ]; then
    exit 1
fi
